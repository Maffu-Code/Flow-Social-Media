...
// Build the base query - include all posts (main posts and replies) for search
$query = "SELECT 
    p.post_id,
    p.content,
    p.parent_post_id,
...
if (!empty($from_date)) {
    $query .= " AND DATE(p.created_at) >= :from_date";
    $params[':from_date'] = $from_date;
}

if (!empty($to_date)) {
    $query .= " AND DATE(p.created_at) <= :to_date";
    $params[':to_date'] = $to_date;
}

$query .= " GROUP BY p.post_id, p.content, p.parent_post_id, p.created_at, u.user_id, u.username, u.display_name, u.avatar
...
$count_query = "SELECT COUNT(DISTINCT p.post_id) as total
FROM posts p
JOIN users u ON p.user_id = u.user_id
WHERE 1=1";
...
foreach ($params as $key => $value) {
    if ($key === ':text') {
        $count_query .= " AND p.content LIKE '$value'";
    } elseif ($key === ':username') {
        $count_query .= " AND u.username LIKE '$value'";
    } elseif ($key === ':from_date') {
        $count_query .= " AND DATE(p.created_at) >= '$value'";
    } elseif ($key === ':to_date') {
        $count_query .= " AND DATE(p.created_at) <= '$value'";
    }
}
