...
// Get all posts (both original posts and first-level replies only)
// This creates a Twitter-like timeline where you see:
// 1. Original posts (parent_post_id IS NULL)
// 2. First-level replies (replies to original posts)
// But NOT second-level replies (replies to replies)
...
$query = "SELECT 
    ...
    CASE WHEN p.parent_post_id IS NOT NULL THEN 
        (SELECT pp.parent_post_id FROM posts pp WHERE pp.post_id = p.parent_post_id LIMIT 1)
        ELSE NULL
    END as parent_is_reply
FROM posts p
JOIN users u ON p.user_id = u.user_id
LEFT JOIN likes l ON p.post_id = l.post_id
LEFT JOIN posts r ON p.post_id = r.parent_post_id
WHERE p.parent_post_id IS NULL 
   OR (p.parent_post_id IS NOT NULL AND 
       (SELECT pp.parent_post_id FROM posts pp WHERE pp.post_id = p.parent_post_id) IS NULL)
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $post = array(
        ...
        "is_first_level_reply" => $row['parent_post_id'] !== null && $row['parent_is_reply'] === null
    );
